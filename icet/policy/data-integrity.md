...
Write의 성공·실패에 따른 갱신 규칙은 아래와 같다. 다른 상태 변경 명령의 갱신 시점과 중첩·동시 실행 명령의 처리 규칙은 아직 설명되지 않았다.
...
### Write 완료 결과에 따른 상태 갱신

2026-09-21 사용자 설명에 따르면 Write 결과에 따라 대상 LBA의 추적 상태를 다음과 같이 처리한다.

| Write 결과 | 대상 LBA의 Data Checker 처리 | 이후 Read |
|---|---|---|
| 성공 | 성공한 Write의 시드로 시드 테이블을 갱신 | 갱신된 시드를 기준으로 데이터 비교 |
| 실패 — 모든 실패 케이스 | `비교 안 함` 상태로 설정 | 해당 상태가 유지되는 동안 데이터 내용 비교 생략 |

새 Write의 시드는 **Write 성공 시에만** 시드 테이블의 예상 값으로 반영한다. 실패 시에는 이전 시드로 계속 비교하지 않고, 해당 명령의 대상 LBA를 `비교 안 함`으로 처리한다. 이는 전체 Namespace 또는 전역 비교 옵션을 끈다는 의미가 아니다.
...
사용자는 모든 실패 케이스에 이 규칙이 적용된다고 설명했다. 타임아웃을 실패로 확정하는 시점, 늦게 도착한 완료와 동시 실행 명령의 처리 순서는 아직 제공되지 않았다.
...
- Write 외 명령의 상태 갱신 시점·실패 처리, 타임아웃 확정·늦은 완료 및 중첩·동시 실행 명령 처리
