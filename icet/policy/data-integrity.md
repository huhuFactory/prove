...
새 예상 상태는 **명령 성공 시에만** 반영한다. 일반 Write는 새 시드를, Write Uncorrectable·Write Zeroes·DSM Trim은 각각에 해당하는 특수 상태를 기록한다. Copy의 구체적인 상태 전달 방식은 추가 설명이 필요하다. 실패 시에는 이전 상태로 계속 비교하지 않고, 해당 명령의 대상 LBA를 `비교 안 함`으로 처리한다. 이는 전체 Namespace 또는 전역 비교 옵션을 끈다는 의미가 아니다.
...
- 반환 데이터의 LBA 정보를 비교해 다른 LBA의 데이터인지 확인한다.
